...
export function DocumentLibrary({ documents, sections }: { documents: LibraryDoc[]; sections: string[] }) {
  ...
  return <section aria-label="Search documents">
    ...
    <div className="document-list">{results.map((doc) => <Link href={doc.href} className="document-card" key={doc.slug}><span className="document-icon" aria-hidden="true">▤</span><div className="document-card-body"><span className="document-section">{doc.section}</span><h2>{doc.title}</h2></div><span className="document-open" aria-hidden="true">↗</span></Link>)}</div>
